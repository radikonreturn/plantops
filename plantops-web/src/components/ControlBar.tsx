...
export function ControlBar({control}: {control: PlantSession}) {
  ...
  return <>
    <header className="app-header">
      ...
      <div className="shift-buttons"><LanguageSelector /><button className="primary" disabled={!!busy || !session || (ended && session.paused)} onClick={() => void control.toggle()}>{ended ? t("Shift complete") : control.connectionHold ? t("Reconnect") : paused ? t("Start shift") : t("Pause")}</button><div className="speed-group" aria-label={t("Playback speed")}>{([1, 2, 4] as const).map(speed => <button key={speed} aria-pressed={session?.speed === speed} disabled={!!busy || !session} onClick={() => void control.speed(speed)}>{speed}×</button>)}</div><button disabled={!!busy} onClick={() => {setSeed(""); setNewShiftOpen(true);}}>{t("New Shift")}</button></div>
    </header>
