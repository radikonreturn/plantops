...
export function ReportsView({session}: {session: SessionSnapshot}) {
  ...
  return <div className="office-view"><div className="view-heading"><h1>{t("Reports")}</h1><span>{s.simulated_minutes >= s.shift_minutes ? t("Shift-end report") : t("Provisional interim report")} · {clock(s.simulated_minutes)}</span></div>
    ...
    {session.decision_review && <section className="work-section"><h2>{t("Decision review")}</h2><dl className="report-readings">
      <div><dt>{t("Events received / answered / ignored")}</dt><dd>{session.decision_review.received} / {session.decision_review.answered} / {session.decision_review.ignored}</dd></div>
      <div><dt>{t("Event decision commitments")}</dt><dd>{money(session.decision_review.decision_cost)}</dd></div>
      <div><dt>{t("Energy surcharge")}</dt><dd>{money(session.decision_review.energy_cost)}</dd></div>
      <div><dt>{t("Customer commitment value")}</dt><dd>{session.decision_review.customer_value}</dd></div>
      <div><dt>{t("Observed delivery / scrap / failures")}</dt><dd>{session.decision_review.observed.delivered} / {session.decision_review.observed.scrap} / {session.decision_review.observed.failures}</dd></div>
      {session.management_objective && <div><dt>{t("Management target")}: {session.management_objective.target_id}</dt><dd>{label(session.management_objective.status)}</dd></div>}
    </dl><p>{t(session.decision_review.note)}</p></section>}
    ...
    <section className="work-section"><h2>{t("Shift event history")}</h2><div className="table-scroll"><table><thead><tr><th>{t("Minute")}</th><th>{t("Event / owner")}</th><th>{t("Lifecycle")}</th><th>{t("Modeled consequence")}</th></tr></thead><tbody>{session.shift_events?.map(e => <tr key={e.id}><td>{e.minute}</td><td>{e.title}<small>{t(e.workspace)} · {e.zone}</small></td><td>{label(e.state)}{e.closed_minute !== null && <small>{t("At {value1}", {value1: clock(e.closed_minute)})}</small>}</td><td>{e.detail}{e.outcome && <p>{e.outcome}</p>}{e.selected_choice && <small>{t("Selected response")}: {e.choices?.find(c => c.id === e.selected_choice)?.label ?? t("Existing service")}</small>}{e.expected && <p>{e.expected}</p>}{e.decision_cost !== undefined && <small>{t("Immediate commitment: {cost}", {cost: e.decision_cost})}</small>}{e.affected_ids.length > 0 && <small>{e.affected_ids.join(", ")}</small>}</td></tr>)}</tbody></table></div></section>
