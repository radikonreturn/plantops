...
export function ReportsView({session}: {session: SessionSnapshot}) {
  ...
  const rows = [
    ...
    ["All-stage scrap", s.total_scrap ?? s.scrap], ["OTIF", percent(s.order_summary.otif)], ["Backlog", `${s.order_summary.backlog_units} units / ${s.order_summary.backlog_orders} orders`],
    ...
    ["Lens / gas cleaning", money(session.cost_breakdown.lens_gas_cleaning ?? 0)],
    ["Chemical / filter service", money(session.cost_breakdown.chemical_filter_service ?? 0)],
    ["Support labor", money(session.cost_breakdown.support_labor ?? 0)],
    ["Tester calibration", money(session.cost_breakdown.tester_calibration ?? 0)],
    ["Rework / retests", s.equipment_quality?.rework ?? 0],
